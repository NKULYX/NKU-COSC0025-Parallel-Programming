...
void initData()
{
    ...
    for(int i=0; i < D; i++)
        data[i] = new float[N], data_align[i] = (float*)_aligned_malloc(sizeof(float)*N, 64);
    for(int i=0; i<D; i++)
        for(int j=0; j<N; j++)
...
void calculate_SSE_aligned()
{
    ...
    {
        __m128 min_dis = _mm_load_ps(tmp);
        for(int j = 0; j < K; j++)
        {
            ...
            for(int d = 0; d < D; d++)
            {
                ...
                __m128 centroid_d = _mm_load_ps(tmp_centroid_d);
                // 一次取出四个元素的某一维度数据
...
void calculate_SSE_unaligned()
{
    ...
    {
        float tmp[4] = {L*L, L*L, L*L, L*L}; 
        __m128 min_dis = _mm_loadu_ps(tmp);
        for(int j = 0; j < K; j++)
        {
            __m128 distance = _mm_setzero_ps();
            for(int d = 0; d < D; d++)
...
void calculate_AVX_aligned()
{
    float tmp[8] = {L*L,L*L,L*L,L*L,L*L,L*L,L*L,L*L}; 
    for(int i = 0; i < N - N % 8; i+=8)
    {
        __m256 min_dis = _mm256_loadu_ps(tmp);
        for(int j = 0; j < K; j++)
        {
            ...
            // 判断当前的每一个数据到该质心的距离是否是最小的
            float disK[8];
            _mm256_storeu_ps(disK, distance);
            for(int k = 0; k < 4; k++)
            {
                if(disK[k] < min_dis[k])
                {
...
void calculate_AVX512_aligned()
{
    ...
    {
        __m512 min_dis = _mm512_loadu_ps(tmp);
        for(int j = 0; j < K; j++)
        {
            ...
            // 判断当前的每一个数据到该质心的距离是否是最小的
            float disK[16];
            _mm512_storeu_ps(disK, distance);
            for(int k = 0; k < 16; k++)
            {
                if(disK[k] < min_dis[k])
                {
